all: bootldr caterpillarAuto ohc ohc-big ohc-arduino-8mhz ohc-arduino-16mhz

.PHONY: docs bootldr caterpillarAuto ohc ohc-big ohc-arduino-8mhz ohc-arduino-16mhz
KILOLIB = build/kilolib.a
bootldr: build/bootldr.elf build/bootldr.hex build/bootldr.lss
caterpillarAuto: build/caterpillarAuto.elf build/caterpillarAuto.hex build/caterpillarAuto.lss
...
build/caterpillarAuto.elf: caterpillarAuto.c $(KILOLIB) | build
	$(CC) $(CFLAGS) -o $@ $< $(KILOLIB) 
...
program-caterpillarAuto: build/caterpillarAuto.hex build/bootldr.hex
	$(AVRUP) -p m328p $(PFLAGS) -U "flash:w:build/caterpillarAuto.hex:i" -U "flash:w:build/bootldr.hex"
